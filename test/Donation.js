...
contract('Donation', async (accounts) => {
    ...
    beforeEach(async function() {
     ...
     it("should let donar to donate", async () => {
        ...
        let donate = await instance.donation("0x0000000000000000000000000000000000000000",{from:accounts[0],value:val});
     ...
     it("should let donar to donate funds and check the reffered address", async () => {
        let instance = await Donation.new();
        let donate = await instance.donation("0x0000000000000000000000000000000000000000",{from:accounts[4],value:val});
     ...
     it("should throw error when donar try to refer himself", async () => {
        try{
            let instance = await Donation.new();
            let donate = await instance.donation("0x0000000000000000000000000000000000000000",{from:accounts[0],value:amount});
    ...
    it("reverts when donar try to donate less than 1 ethers", async () => {
        try{
            let instance = await Donation.new();
            let donate = await instance.donation("0x0000000000000000000000000000000000000000",{from:accounts[0],value:99999999999999999});
            let donate1 = await instance.donation(accounts[0],{from:accounts[1],value:val});
            let referral = await instance.referred.call(accounts[1]);    
            }
        catch(err){
